...
- Experimental cookie controls (hidden flags/env):
  ...
  - Practical minimal set that keeps ChatGPT logged in and avoids the workspace picker: `__Secure-next-auth.session-token` (include `.0`/`.1` variants), `_account` (active workspace/account), and any active Cloudflare proofs (`cf_clearance`, `__cf_bm`/`_cfuvid`/`CF_Authorization`/`__cflb`). Everything else re-mints after login.
